...
namespace KCVLoggerPlugin.Models
{
	...
	/// </remarks>
	public class MissionLog : LogBase<MissionLogStruct>
	{
		/*
		...
		/// </value>
		public new static MissionLog Instance
		{
			get
			{
				if (instance == null)
				{
					lock (lockForSingleton)
					{
						if (instance == null)
						{
							...
							instance = new MissionLog();
						}
					}
				}
				// instance フィールドに格納されている型は基底クラスのため、
				// 派生クラスにキャストして返す。
				return (MissionLog)instance;
			}
		}
		...
		{
			ObservableCollection<MissionLogStruct> data = new ObservableCollection<MissionLogStruct>();
		...
		public override void ExportCsv(String path)
		{
			...
			foreach (MissionLogStruct bl in this.History)
			{
