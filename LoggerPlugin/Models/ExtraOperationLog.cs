...
namespace KCVLoggerPlugin.Models
{
    ...
    /// </remarks>
    public class ExtraOperationLog : LogBase<ExtraOperationLogStruct>
    {
        /*
        ...
        /// </value>
        public new static ExtraOperationLog Instance
        {
            get
            {
                if (instance == null)
                {
                    lock (lockForSingleton)
                    {
                        if (instance == null)
                        {
                            ...
                            instance = new ExtraOperationLog();
                        }
                    }
                }
                // instance フィールドに格納されている型は基底クラスのため、
                // 派生クラスにキャストして返す。
                return (ExtraOperationLog)instance;
            }
        }
        ...
        {
            ObservableCollection<ExtraOperationLogStruct> data = new ObservableCollection<ExtraOperationLogStruct>();
			...
			while (parser.Read())
			{
                ...
                // データ格納
                data.Add(new ExtraOperationLogStruct(
        ...
        public override void ExportCsv(String path)
        {
            ...
            foreach (ExtraOperationLogStruct bl in this.History)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public int GetThisMonthEOAachievement()
        {
            ExtraOperationLogStruct thisMonthLog;
            thisMonthLog = Instance.History.ToList().Find(log => (log.DateTime == new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1)) );
            if (thisMonthLog == null)
            {
                return 0;
            }
            return thisMonthLog.GetAachievementSum();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public int GetLastMonthEOAachievement()
        {
            ExtraOperationLogStruct lastMonthLog;
            lastMonthLog = Instance.History.ToList().Find(log => (log.DateTime == new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1).AddMonths(-1)));
            if (lastMonthLog == null)
            {
                return 0;
            }
            return lastMonthLog.GetAachievementSum();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public int GetTodayMonthEOAachievement()
        {
            ExtraOperationLogStruct thisMonthLog;
            thisMonthLog = Instance.History.ToList().Find(log => (log.DateTime == new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1)));
            if (thisMonthLog == null)
            ...
            return thisMonthLog.GetAachievementSumToday();
        }
    }
}
