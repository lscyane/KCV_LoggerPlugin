...
namespace KCVLoggerPlugin.Models
{
    ...
    /// </remarks>
    public class CreateShipLog : LogBase<CreateShipLogStruct>
    {
        /*
        ...
        /// </value>
        public new static CreateShipLog Instance
        {
            get
            {
                if (instance == null)
                {
                    lock (lockForSingleton)
                    {
                        if (instance == null)
                        {
                            ...
                            instance = new CreateShipLog();
                        }
                    }
                }
                // instance フィールドに格納されている型は基底クラスのため、
                // 派生クラスにキャストして返す。
                return (CreateShipLog)instance;
            }
        }
        ...
        {
            ObservableCollection<CreateShipLogStruct> data = new ObservableCollection<CreateShipLogStruct>();
			...
			while (parser.Read())
			{
				...
				// データ格納
				data.Add(new CreateShipLogStruct(
                    ...
                    Int32.Parse(mediumClass[7]),
                    mediumClass[8],
                    Int32.Parse(mediumClass[9]),
                    true
                    ));
            }
			parser.Dispose();
        ...
        public override void ExportCsv(String path)
        {
            ...
            foreach (CreateShipLogStruct bl in this.History)
            {
